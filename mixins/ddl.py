...
from trino_ops_utils.configs.ops.ddl.schema import (
    CreateSchema,
    DropSchema
)

from trino_ops_utils.configs.ops.ddl.table import (
    CreateTable,
    DropTable,
    RenameTable
)

from trino_ops_utils.mixins.interface import (
...
class CreateSchemaMixin(HasSchemaTableObjects, BuildsOp, ExecutesOp):
    ...
    Expects the host class to provide:
        - ``self.obj_schema``: a :class:`~trino_ops_utils.configs.TrinoSchema` instance
...
class CreateTableMixin(HasSchemaTableObjects, BuildsOp, ExecutesOp):
    ...
    Expects the host class to provide:
        - ``self.obj_table``: a :class:`~trino_ops_utils.configs.TrinoTableDDL` instance
        - ``make_op(...)`` and ``execute_op(...)`` helpers (see :class:`RenderExecMixin`)
